...
function _getUser(id, cb) {
  ...
  _getUserByQuery(function (query) {
    query.equalTo('username', id);
  }, cb);
}

function _getUserByQuery(filter, cb) {
...
function getUserIdByGoogleUserId(googleUserId, cb) {
  _getUserByQuery(function (query) {
    query.equalTo('linkedProfiles.google.id', googleUserId);
  }, function (err, parseUser) {
    if (err) { return cb(err); }
    if (!parseUser) {
      return cb(null, null);
    }
    cb(err, _toPlainObject(parseUser, 'username').id);
  });
}
...
module.exports = {
  ...
  getUserIdByGoogleUserId : getUserIdByGoogleUserId
};
