...
module.exports = {
  name : 'MongoDataStore',
  get : requireDb(function (selector, cb) {
  ...
  insert : requireDb(function (selector, entity, cb) {
  ...
  update : requireDb(function (selector, entity, cb) {
    // TODO: Return an error if selector / filter not provided
    var collection = Object.keys(selector)[0];
    var filter = selector[collection];
    entity = clone(entity);
  ...
  delete : requireDb(function (selector, cb) {
    var collection = Object.keys(selector)[0];
    var filter = selector[collection];
    // TODO: Return an error if selector / filter not provided
